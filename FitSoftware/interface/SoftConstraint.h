...
class SoftConstraint : public Constraint{
 public:
  SoftConstraint(std::string _name, unsigned int _idx, TMatrixT<double> *_val, TMatrixTSym<double> *_cov, TMatrixT<double> *_dalpha, double _initalSigma, double _sigma_min, unsigned int _maxIterations=5, double _scalefactor=0.75);
  virtual ~SoftConstraint();

  virtual void addCorrelation(int idx){if(idx<cov_->GetNrows())corridx_.push_back(idx);}
